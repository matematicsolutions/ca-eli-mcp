...
from ca_eli_mcp.coverage import build_coverage
from ca_eli_mcp.server import ca_coverage


def _coverage():
    fn = getattr(ca_coverage, "fn", ca_coverage)
...
def test_tool_writes_an_audit_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CA_ELI_AUDIT_DIR", str(tmp_path))
    _coverage()
    lines = [
        line
    ...
    assert any("ca_coverage" in line for line in lines), (
        "an audit entry was written but it does not name ca_coverage"
    )
