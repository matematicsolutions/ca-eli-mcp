...
SOURCE = 'laws-lois.justice.gc.ca (Justice Laws)'
...
_FAMILIES: list[dict] = [{'name': 'Federal legislation', 'tool': 'ca_get_document / ca_get_text'}]

_GAPS: list[dict] = [{'id': 'CA-001', 'family': 'Federal legislation', 'missing': 'Provincial and territorial law is out of scope - this is federal legislation only.', 'fallback': 'Use the relevant province or territory legislation site.'}, {'id': 'CA-002', 'family': 'Federal legislation', 'missing': 'There is no full-text search across Canadian law here; documents are reached by their coordinate.', 'fallback': 'Use the Justice Laws website search to find an identifier first.'}, {'id': 'CA-003', 'family': 'Federal legislation', 'missing': 'Legal force on a given date is not evaluated. Text is returned as the source publishes it; this connector does not decide whether a provision is in force.', 'fallback': 'Check the validity or consolidation dates on the source record itself.'}]
